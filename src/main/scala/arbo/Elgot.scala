...
import higherkindness.droste.{Algebra, Coalgebra, Embed, GAlgebra, GCoalgebra, kernel}
...
package object elgot {
  ...
  object ElgotCoalgebra extends scala.AnyRef {
    def apply[E[_], F[_], A](f: scala.Function1[A, E[F[A]]]): ElgotCoalgebra[E, F, A] =
      GCoalgebra[Lambda[a => E[F[a]]], A, A](f)
  }

  object ElgotAlgebra extends scala.AnyRef {
    def apply[E[_], F[_], A](f: scala.Function1[E[F[A]], A]): ElgotAlgebra[E, F, A] = GAlgebra[Lambda[a => E[F[a]]], A, A](f)
  }
