...
object SellTree {
  case class SellNode[+A](order: SellOrder, depth: Depth, children: NonEmptyList[A])
      extends SellTree[A]
  ...
  implicit val SellTreeTraverse: Traverse[SellTree] = new Traverse[SellTree] {
    override def traverse[G[_]: Applicative, A, B](fa: SellTree[A])(f: A => G[B]): G[SellTree[B]] =
      fa match {
    ...
    override def foldRight[A, B](fa: SellTree[A], lb: Eval[B])(
